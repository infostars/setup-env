...
# 64-bit RHEL/CentOS 6 only: libstdc++ library from a gcc 4.9.2 source build.
...
# $1 = Original $0 (i.e. script name)
{
   ...
   # Mirror core URL
   coreurl="http://mirror.centos.org/centos-7/$centver"

   # CentOS $centver updates directory
   baseurl="$coreurl/updates/x86_64/Packages"

   # CentOS $centver initial release directory
   baseurlfallback="$coreurl/os/x86_64/Packages"
...
{
   download_file "$checksite$download_lib_xz" "$download_lib_xz" "1476351727 231052 $download_lib_xz" 1
...
# Extract and install libraries from CentOS 7 RPMs
{
   ...
   *)
   ...
   install_library  "libstdc++-4.8.2-16.2.el7_0"   2441727600     308620    "$relusrlib/libstdc++.so.6"  "$baseurl"             0
   install_library  "libstdc++-4.8.2-16.el7"        844211201     308232    "$relusrlib/libstdc++.so.6"  "$baseurlfallback"     1
   ;;
   esac
}
